                ...
                <div class="col-md-6">
                    ...
                    <datalist id="tagihan_list">
                        @foreach($tagihansUntukInput as $tagihan)
                            <option value="{{ $tagihan->nama_instansi }} | Invoice {{ $tagihan->no_invoice }} | {{ $tagihan->nama_bulan_tagihan }} {{ $tagihan->tahun_tagihan }} | Rp {{ number_format($tagihan->total_bayar,0,',','.') }}" data-id="{{ $tagihan->id }}" data-nominal="{{ $tagihan->total_bayar }}"></option>
                        @endforeach
                    </datalist>
                    <div class="form-text">Ketik untuk mencari pelanggan. Data yang sudah ditugaskan tidak akan tampil.</div>
                </div>
                <div class="col-md-3">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        tagihanInput.addEventListener('input', () => {
            ...
            const opsi = Array.from(tagihanList).find((option) => option.value === value);
